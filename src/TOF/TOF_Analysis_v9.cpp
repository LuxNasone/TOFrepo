...
static const long long FIFO_COINC_WINDOW = 4000000000LL;
...
static long long CalcExpectedFifoTs(double drs_fpga_equiv, double fpga_start_s) {
...
static bool FifoCoincidenceCheck(const FifoData &fd, long long ts_expected,
                                  long long &last_used_ts1) {

    if (ts_expected < 0LL) return false;
    if (fd.ts.empty())     return false;

    // ---- Ricerca nella finestra [ts_lo, ts_hi] per canale 1 ----
    ...
    for (auto it = it2_lo; it != fd.ts.end() && *it <= ts2_hi; ++it) {
        size_t idx = (size_t)(it - fd.ts.begin());
        if (fd.chan[idx] == 2) {
            if (ts1_found == last_used_ts1) return false;
            last_used_ts1 = ts1_found;
            return true;
        }
    }
...
// ==========================================================================
//  FindBetaExtreme — Zero crossing (su istogramma qualsiasi) + β_min (da h_beta)
// ==========================================================================
//
//  SCOPO:
//    1) Zero crossing che corrisponde a β max, calcolato sulla differenza di due istogrammi qualsiasi 
//    2) 1/β_zero_exit = primo zero crossing da sinistra, corrisponde a β minimo
//
//  ARGOMENTI:
//    file1, file2        — percorsi dei file ROOT
//    hist_name           — nome dell'istogramma per lo zero crossing
//    soglia_conteggi     — conteggi minimi per β_min su h_beta (default=50)
//    soglia_rumore       — soglia per lo zero crossing (default=1e-6)
//
//  RESTITUISCE:
//    zero_crossing (in unità dell'istogramma scelto), oppure -1.0 in caso di errore
//    file root con grafici
//  USO:
//     FindBetaExtreme("TOF_Analysis_output_prova1.root","TOF_Analysis_output_prova2.root","h_inv_beta_tot",20,1e-1)
// ==========================================================================

double FindBetaExtreme(const char* file1,
                       const char* file2,
                       const char* hist_name,
                       int soglia_conteggi = 50,
                       double soglia_rumore = 1e-6) {

    std::cout << "\n=============================================" << std::endl;
    std::cout << "  FindBetaExtreme" << std::endl;
    std::cout << "=============================================" << std::endl;
    std::cout << "  File1: " << file1 << std::endl;
    std::cout << "  File2: " << file2 << std::endl;
    std::cout << "  Zero crossing su: " << hist_name << std::endl;
    std::cout << "  Soglia rumore (zero crossing): " << soglia_rumore << std::endl;
    ...
    if (!f1 || f1->IsZombie() || !f2 || f2->IsZombie()) {
        std::cerr << "[ERRORE] Impossibile aprire i file." << std::endl;
        if (f1) f1->Close(); if (f2) f2->Close();
        return -1.0;
    }

    TCanvas *c_scatter = nullptr;
    TH1D *h_mass = nullptr;

    // ============================================================
    //  PARTE 1: Zero crossing sull'istogramma richiesto
    // ============================================================
    TH1D *h1 = (TH1D*)f1->Get(hist_name);
    TH1D *h2 = (TH1D*)f2->Get(hist_name);

    if (!h1 || !h2) {
        std::cerr << "[ERRORE] Istogramma '" << hist_name << "' non trovato." << std::endl;
        f1->Close(); f2->Close();
        return -1.0;
    }

    if (h1->GetNbinsX() != h2->GetNbinsX()) {
        std::cerr << "[ERRORE] Binning diverso per " << hist_name << std::endl;
        f1->Close(); f2->Close();
        return -1.0;
    }

    TH1D *hdiff = (TH1D*)h1->Clone("hdiff_temp");
    hdiff->Add(h2, -1.0);

    double zero_crossing = -1.0;
    bool found = false;

    for (int i = nbins; i >= 2; i--) {
        double d1 = hdiff->GetBinContent(i-1);
        double d2 = hdiff->GetBinContent(i);
        double x1 = hdiff->GetBinCenter(i-1);
        double x2 = hdiff->GetBinCenter(i);

        if ((d1 * d2) < 0 && fabs(d1) > soglia_rumore && fabs(d2) > soglia_rumore) {
            zero_crossing = x1 - d1 * (x2 - x1) / (d2 - d1);
            found = true;
            break;
        }
    }
    ...
    if (!found) {
        std::cerr << "[ERRORE] Nessuno zero crossing trovato su " << hist_name << std::endl;
        f1->Close(); f2->Close();
        return -1.0;
    }

    std::cout << "\n>>> Zero crossing su " << hist_name << ":" << std::endl;
    std::cout << "    x_zero = " << zero_crossing << std::endl;
    std::cout << "=============================================" << std::endl;

    // ============================================================
    //  PARTE 2: Ultimo zero prima di differenze diverse da zero
    // ============================================================
    double last_zero_before_nonzero = -1.0;
    bool found_exit_zero = false;
    
    // Partiamo da destra e cerchiamo l'ultimo bin con differenza ≈ zero
    // che sia seguito (a destra) da differenze diverse da zero
    for (int i = nbins - 1; i >= 1; i--) {
        double d_current = hdiff->GetBinContent(i);
        double d_next = hdiff->GetBinContent(i+1);
        
        // Controlla se current è ≈ zero e next è significativamente diverso da zero
        bool current_is_zero = (fabs(d_current) <= soglia_rumore);
        bool next_is_nonzero = (fabs(d_next) > soglia_rumore);
        
        if (current_is_zero && next_is_nonzero) {
            last_zero_before_nonzero = hdiff->GetBinCenter(i);
    ...
    if (found_exit_zero) {
        std::cout << "\n>>> Ultimo zero prima di differenze ≠ 0 (uscita dallo zero):" << std::endl;
        std::cout << "    1/β_zero_exit = " << last_zero_before_nonzero << std::endl;
        if (strcmp(hist_name, "h_invv") == 0) {
            std::cout << "    1/β_zero_exit = " << 1.0/last_zero_before_nonzero << std::endl;
        }
    } else {
        std::cout << "\n[WARNING] Nessun 'uscita dallo zero' trovata." << std::endl;
    }

    // ============================================================
    //  PARTE 3: Fit gaussiano sui due istogrammi originali
    // ============================================================
    // std::cout << "\n>>> PARTE 3: Fit gaussiano su " << hist_name << std::endl;
    
    // // --- Trova il picco per il range di fit ---
    // int bin_max1 = h1->GetMaximumBin();
    // int bin_max2 = h2->GetMaximumBin();
    // double x_peak1 = h1->GetBinCenter(bin_max1);
    // double x_peak2 = h2->GetBinCenter(bin_max2);
    
    // // Range di fit: ±3 sigma approssimato (usa la larghezza a metà altezza)
    // // Alternativa: range fisso di ±0.3 se non conosci la larghezza
    // double fit_range = 0.3;  // modifica in base ai tuoi dati
    // double fit_lo1 = std::max(x_peak1 - fit_range, h1->GetXaxis()->GetXmin());
    // double fit_hi1 = std::min(x_peak1 + fit_range, h1->GetXaxis()->GetXmax());
    // double fit_lo2 = std::max(x_peak2 - fit_range, h2->GetXaxis()->GetXmin());
    // double fit_hi2 = std::min(x_peak2 + fit_range, h2->GetXaxis()->GetXmax());
    
    // // --- Fit per h1 ---
    // TF1 *gaus1 = new TF1("gaus1", "gaus", fit_lo1, fit_hi1);
    // gaus1->SetParameters(h1->GetMaximum(), x_peak1, 0.05);
    // h1->Fit(gaus1, "QRS0");
    
    // // --- Fit per h2 ---
    // TF1 *gaus2 = new TF1("gaus2", "gaus", fit_lo2, fit_hi2);
    // gaus2->SetParameters(h2->GetMaximum(), x_peak2, 0.05);
    // h2->Fit(gaus2, "QRS0");
    
    // // --- Stampa risultati ---
    // std::cout << "Fit h1 (" << hist_name << ", file1):" << std::endl;
    // std::cout << "    μ = " << gaus1->GetParameter(1) << " ± " << gaus1->GetParError(1) << std::endl;
    // std::cout << "    σ = " << fabs(gaus1->GetParameter(2)) << " ± " << gaus1->GetParError(2) << std::endl;
    // std::cout << "Fit h2 (" << hist_name << ", file2):" << std::endl;
    // std::cout << "    μ = " << gaus2->GetParameter(1) << " ± " << gaus2->GetParError(1) << std::endl;
    // std::cout << "    σ = " << fabs(gaus2->GetParameter(2)) << " ± " << gaus2->GetParError(2) << std::endl;
    
    // // --- Canvas con i due istogrammi e i fit ---
    // TCanvas *c_fit = new TCanvas("c_fit", "Fit gaussiano", 900, 600);
    // c_fit->SetGrid();
    
    // h1->SetLineColor(kRed);
    // h1->SetLineWidth(2);
    // h1->SetTitle(Form("Distribuzione %s e fit gaussiano", hist_name));
    // h1->Draw("HIST");
    ...
    // // Legenda
    // TLegend *leg_fit = new TLegend(0.65, 0.70, 0.89, 0.88);
    // leg_fit->SetTextFont(42);
    // leg_fit->SetTextSize(0.030);
    // leg_fit->AddEntry(h1, Form("File1: μ = %.4f", gaus1->GetParameter(1)), "l");
    // leg_fit->AddEntry(h2, Form("File2: μ = %.4f", gaus2->GetParameter(1)), "l");
    // leg_fit->Draw();
    
    // c_fit->Write("Canvas_Fit");

    

    // ============================================================
    //  PARTE 3: Calcolo delle masse dai valori di 1/β tra i due zero crossing
    // ============================================================
    
    // --- Costanti fisiche (dimensioni: MeV/c^2 per masse, tutto coerente) ---
    const double Z = 82.0;                      // numero atomico del piombo
    const double z = 1.0;                      // carica della particella (muone)
    const double K = 0.307;                    // [MeV cm^2 / mol] costante di Bethe-Bloch
    const double A = 207.2;                    // [g/mol] massa molare del piombo
    const double I = 823.0;                    // [eV] potenziale di ionizzazione medio (convertito in MeV)
    const double I_MeV = I * 1e-6;             // [MeV] 823 eV = 0.000823 MeV
    const double m_e = 0.511;                  // [MeV/c^2] massa dell'elettrone
    const double rho = 11.35;                  // [g/cm^3] densità del piombo
    const double c_light = 29.9792458;         // [cm/ns] velocità della luce (per β)
    
    // --- Limiti e soglie ---
    const double M_MIN = 1.0;                 // [MeV/c^2] massa minima (muone)
    const double M_MAX = 500.0;                // [MeV/c^2] massa massima per la ricerca
    const double M_STEP = 1.0;                 // passo di ricerca [MeV/c^2]
    const double R_TOL = 1e-1;                 // tolleranza per R ≈ 1
    
    std::cout << "\n>>> PARTE 3: Calcolo masse da valori 1/β tra i due zero crossing" << std::endl;
    std::cout << "    Range 1/β: [" << last_zero_before_nonzero << ", " << zero_crossing << "]" << std::endl;
    
    // --- Vettori per i risultati ---
    std::vector<double> velocita;       // β (adimensionale)
    std::vector<double> masse;          // M [MeV/c^2]
    
    // --- Funzione per calcolare dE/dx (Bethe-Bloch) in MeV/cm ---
    // Formula: dE/dx = (Z*z^2*K)/(β^2 * A) * [1/2 * ln((2*β^2*γ^2*m_e*c^2*T_max)/I^2) - β^2] * ρ
    // T_max = (2*β^2*γ^2*m_e*c^2) / (1 + 2*γ*m_e/M + (m_e/M)^2) ≈ 2*β^2*γ^2*m_e*c^2 per M >> m_e
    auto BetheBloch = [&](double beta, double M_MeV) -> double {
        double gamma = 1.0 / sqrt(1.0 - beta*beta);
        // Approssimazione T_max per particella pesante (M >> m_e)
        double T_max = 2.0 * beta*beta * gamma*gamma * m_e;  // [MeV] (c^2 = 1 implicito nelle unità)
        
        double ln_term = 0.5 * log((2.0 * beta*beta * gamma*gamma * m_e * T_max) / (I_MeV * I_MeV));
        double beta2 = beta*beta;
        
        double dEdx = (Z * z*z * K) / (beta2 * A) * (ln_term - beta2) * rho;  // [MeV/cm]
        return dEdx;
    };
    
    // --- Funzione per calcolare R = (M * γ * c^2) / (dE/dx) ---
    // Attenzione: γ * c^2 ha dimensioni di [c^2], ma dE/dx è in [MeV/cm].
    // La formula è puramente adimensionale se M è in [MeV/c^2] e dE/dx in [MeV/cm]
    // ma richiede una lunghezza caratteristica. Qui assumiamo che il risultato
    // sia adimensionale (R confrontato con 1).
    auto CalculateR = [&](double beta, double M_MeV) -> double {
        double gamma = 1.0 / sqrt(1.0 - beta*beta);
        double dEdx = BetheBloch(beta, M_MeV);
        if (dEdx <= 0.0) return -1.0;
        // R = (M * γ * c^2) / (dE/dx) - dimensionalmente:
        // M [MeV/c^2] * γ * c^2 [c^2] = M*γ [MeV] diviso dE/dx [MeV/cm] → [cm]
        // Questo è uno spessore. Per confrontarlo con 1 (adimensionale), assumiamo
        // che lo spessore target sia 1 cm. Quindi R = (M*γ) / (dE/dx*1cm).
        double R = (M_MeV * gamma) / dEdx;
        return R;
    };
    
    // --- Raccogli i valori di 1/β nell'intervallo [zero_exit, zero_crossing] ---
    // NOTA: zero_exit < zero_crossing (perché 1/β cresce verso destra)
    double invbeta_lo = std::min(last_zero_before_nonzero, zero_crossing);
    double invbeta_hi = std::max(last_zero_before_nonzero, zero_crossing);
    
    std::vector<double> invbeta_values;
    
    for (int i = 1; i <= nbins; i++) {
        double invbeta = hdiff->GetBinCenter(i);
        if (invbeta >= invbeta_lo && invbeta <= invbeta_hi) {
            invbeta_values.push_back(invbeta);
        }
    }
    
    std::cout << "    Trovati " << invbeta_values.size() << " bin nell'intervallo." << std::endl;
    
    // --- Per ogni valore di 1/β, converti in β e calcola M ---
    for (size_t idx = 0; idx < invbeta_values.size(); idx++) {
        double invbeta = invbeta_values[idx];
        double beta = 1.0 / invbeta;          // β = 1 / (1/β)
        
        // Controllo fisico: β deve essere tra 0 e 1
        if (beta <= 0.0 || beta >= 1.0) {
            std::cout << "    β = " << beta << " non fisico, skip." << std::endl;
            continue;
        }
        
        velocita.push_back(beta);

        if (idx == 0) {
            // Debug: calcola R per beta medio con M=100
            double beta_test = velocita[0];  // primo beta
            double gamma_test = 1.0 / sqrt(1.0 - beta_test*beta_test);
            double dEdx_test = BetheBloch(beta_test, 100.0);
            double R_test = (100.0 * gamma_test) / dEdx_test;
            std::cout << "DEBUG: beta=" << beta_test << " dEdx=" << dEdx_test << " R=" << R_test << std::endl;
            std::cout << "Per avere R=1, servirebbe M = " << dEdx_test / gamma_test << std::endl;
        }
        
        // --- Cerca M tale che R = 1 (entro tolleranza) ---
        double M_found = -1.0;
        
        for (double M = M_MIN; M <= M_MAX; M += M_STEP) {
            double R = CalculateR(beta, M);
            if (R > 0.0 && fabs(R - 1.0) <= R_TOL) {
                M_found = M;
                break;
            }
        }
        ...
        if (M_found > 0) {
            masse.push_back(M_found);
            std::cout << "    β = " << beta << " → M = " << M_found << " MeV/c^2 (R = " 
                      << CalculateR(beta, M_found) << ")" << std::endl;
        } else {
            std::cout << "    β = " << beta << " → nessuna M trovata in [" 
                      << M_MIN << ", " << M_MAX << "]" << std::endl;
            masse.push_back(-1.0);  // valore sentinella
        }
    }
    
    // --- Scatter plot: velocità (β) vs massa (M) ---
    if (!velocita.empty() && !masse.empty()) {
        c_scatter = new TCanvas("c_scatter", "Massa vs Velocità", 900, 600);
        c_scatter->SetGrid();
        
        TGraph *gr = new TGraph((int)velocita.size());
        gr->SetTitle("Massa ricostruita vs #beta; #beta; M [MeV/c^{2}]");
        gr->SetMarkerStyle(20);
        gr->SetMarkerSize(0.8);
        gr->SetMarkerColor(kBlue);
        
        int n_valid = 0;
        for (size_t i = 0; i < velocita.size(); i++) {
            if (masse[i] > 0) {
                gr->SetPoint(n_valid, velocita[i], masse[i]);
                n_valid++;
            }
        }
        ...
        if (n_valid > 0) {
            gr->Set(n_valid);
            gr->Draw("AP");
            c_scatter->Write("Canvas_Mass_vs_Beta");
        } else {
            std::cout << "[WARNING] Nessun punto valido per lo scatter plot." << std::endl;
        }
        
        // --- Opzionale: istogramma delle masse trovate ---
        h_mass = new TH1D("h_mass", "Distribuzione masse ricostruite; M [MeV/c^{2}]; Conteggi", 100, M_MIN, M_MAX);
        for (size_t i = 0; i < masse.size(); i++) {
            if (masse[i] > 0) h_mass->Fill(masse[i]);
        }
        h_mass->Write();
        
    } else {
        std::cout << "[WARNING] Nessun dato per lo scatter plot." << std::endl;
    }
    
    std::cout << "=============================================" << std::endl;

    // --- Canvas ---
    TCanvas *c = new TCanvas("c_result", "Zero crossing", 900, 600);
    c->SetGrid();
    double ymax = std::max(fabs(hdiff->GetMaximum()), fabs(hdiff->GetMinimum()));
    hdiff->GetYaxis()->SetRangeUser(-ymax*1.1, ymax*1.1);
    hdiff->SetTitle(Form("Differenza (%s - %s);%s;Conteggi", 
                  hist_name, hist_name, hdiff->GetXaxis()->GetTitle()));
    hdiff->SetLineColor(kRed);
    hdiff->Draw("E1");
    TLine *line = new TLine(zero_crossing, -ymax*0.9, zero_crossing, ymax*0.9);
    line->SetLineColor(kBlue); line->SetLineWidth(2); line->SetLineStyle(2);
    line->Draw("same");
    c->Write("Canvas_ZeroCrossing");
        // --- Salva in file ROOT ---
    TFile *fout = new TFile("zero_crossing_output.root", "RECREATE");
    c->Write();                    // salva il canvas
    //c_fit->Write();                // salva il canvas dei fit
    if (c_scatter) c_scatter->Write();
    hdiff->Write("hdiff");         // salva l'istogramma differenza
    if (h_mass) h_mass->Write();
    fout->Close();
    f1->Close(); f2->Close();
...
void TOF_Analysis(const char* xml_files,
                  const char* outname  = "TOF_Analysis_output_prova1.root",
                  const char* cal_file = "") {
...
// Indice del file sorgente (per tracciabilità)
    ...
    TH1D *h_beta_norm   = new TH1D("h_beta_norm",   "Distribuzione #beta normalizzata = v/c;#beta;Conteggi",
                               NBINS_BETA, BETA_LO, BETA_HI);      
    TH1D *h_inv_beta = new TH1D("h_inv_beta", "Distribuzione 1/#beta;1/#beta;Conteggi",
                             NBINS_BETA, BETA_LO, BETA_HI);                     
...
// Istogramma 2D: beta vs x_imp
    ...
    TH1D *h_beta_norm_tot = new TH1D("h_beta_norm_tot",
                                   ...
                                   "#beta;Conteggi",
                                   NBINS_BETA, BETA_LO, BETA_HI);
    TH1D *h_inv_beta_tot = new TH1D("h_inv_beta_tot",
                                 "Distribuzione 1/#beta (TOT);1/#beta;Conteggi",
                                 NBINS_BETA, BETA_LO, BETA_HI);
...
// ============================================================
            ...
            if (good_tot == 1) {
                ...
                if (tof_tot > 0.1) {
                    ...
                    if (beta_tot > 0 && beta_tot < 3.0) {
                        ...
                        h_inv_beta_tot->Fill(1.0 / beta_tot);
                    }
    ...
    if (h_inv_beta->Integral() > 0.0) {
        double norm_inv_beta = h_inv_beta->Integral("width");
        h_inv_beta->Scale(1.0 / norm_inv_beta);
        h_inv_beta->GetYaxis()->SetTitle("PDF [1/unit. 1/#beta]");
        std::cout << "[INFO] h_inv_beta normalizzata all'area unitaria."
              << "  Fattore = " << norm_inv_beta << std::endl;
    } else {
        std::cout << "[WARNING] h_inv_beta vuota: normalizzazione saltata." << std::endl;
    }        
    ...
    if (h_inv_beta_tot->Integral() > 0.0) {
    double norm_inv_beta_tot = h_inv_beta_tot->Integral("width");
    h_inv_beta_tot->Scale(1.0 / norm_inv_beta_tot);
    h_inv_beta_tot->GetYaxis()->SetTitle("PDF [1/unit. 1/#beta]");
    } else {
    std::cout << "[WARNING] h_inv_beta_tot vuota: normalizzazione saltata." << std::endl;
    }
    // ==================================================================
    ...
    // --- Canvas 9a: PDF di 1/β  ---
    TCanvas *c_inv_beta = new TCanvas("c_inv_beta", "Distribuzione 1/#beta", 900, 650);
    c_inv_beta->SetGrid();
    h_inv_beta->SetLineColor(kRed + 1);
    h_inv_beta->SetLineWidth(2);
    h_inv_beta->Draw("HIST");

    TLine *l_inv_beta1 = new TLine(1.0, 0.0, 1.0, h_inv_beta->GetMaximum() * 0.95);
    l_inv_beta1->SetLineColor(kBlack);
    l_inv_beta1->SetLineStyle(2);
    l_inv_beta1->SetLineWidth(2);
    l_inv_beta1->Draw("same");

    TLatex *lat_inv_beta = new TLatex(1.02, h_inv_beta->GetMaximum() * 0.80, "1/#beta = 1");
    lat_inv_beta->SetTextSize(0.035);
    lat_inv_beta->Draw("same");

    TPaveText *pt_inv_beta = new TPaveText(0.60, 0.72, 0.89, 0.88, "NDC");
    pt_inv_beta->SetFillColor(0);
    pt_inv_beta->SetBorderSize(1);
    pt_inv_beta->SetTextAlign(12);
    pt_inv_beta->SetTextFont(42);
    pt_inv_beta->SetTextSize(0.030);
    pt_inv_beta->AddText(Form("Entries = %lld", (Long64_t)h_inv_beta->GetEntries()));
    pt_inv_beta->AddText("#int PDF(1/#beta) d(1/#beta) = 1");
    pt_inv_beta->Draw();

    c_inv_beta->Write("Canvas_inv_beta");

    // --- Canvas 9b: PDF di 1/β  per il metodo TOT ---
    TCanvas *c_inv_beta_tot = new TCanvas("c_inv_beta_tot", "Distribuzione 1/#beta (TOT)", 900, 650);
    c_inv_beta_tot->SetGrid();
    h_inv_beta_tot->SetLineColor(kBlue + 1);
    h_inv_beta_tot->SetLineWidth(2);
    h_inv_beta_tot->Draw("HIST");

    TLine *l_inv_beta1_tot = new TLine(1.0, 0.0, 1.0, h_inv_beta_tot->GetMaximum() * 0.95);
    l_inv_beta1_tot->SetLineColor(kBlack);
    l_inv_beta1_tot->SetLineStyle(2);
    l_inv_beta1_tot->SetLineWidth(2);
    l_inv_beta1_tot->Draw("same");

    TLatex *lat_inv_beta_tot = new TLatex(1.02, h_inv_beta_tot->GetMaximum() * 0.80, "1/#beta = 1");
    lat_inv_beta_tot->SetTextSize(0.035);
    lat_inv_beta_tot->Draw("same");

    TPaveText *pt_inv_beta_tot = new TPaveText(0.60, 0.72, 0.89, 0.88, "NDC");
    pt_inv_beta_tot->SetFillColor(0);
    pt_inv_beta_tot->SetBorderSize(1);
    pt_inv_beta_tot->SetTextAlign(12);
    pt_inv_beta_tot->SetTextFont(42);
    pt_inv_beta_tot->SetTextSize(0.030);
    pt_inv_beta_tot->AddText(Form("Entries = %lld", (Long64_t)h_inv_beta_tot->GetEntries()));
    pt_inv_beta_tot->AddText("#int PDF(1/#beta) d(1/#beta) = 1");
    pt_inv_beta_tot->Draw();

    c_inv_beta_tot->Write("Canvas_inv_beta_TOT");
...
void TOF_Analysis_lead(const char* xml_files,
    ...
    int rej_x_out_tot       = 0;
    int rej_tof_unphys_tot  = 0;
    int total_good_lead     = 0;    // eventi buoni (slew) che passano anche il FIFO
    int total_good_tot_lead = 0;    // eventi buoni (TOT) che passano anche il FIFO
    
    long long last_used_ts1 = -1LL;
    ...
    for (size_t fi = 0; fi < file_list.size(); fi++) {
        ...
        for (size_t ev = 0; ev < events.size(); ev++) {
            ...
            if (ts_exp < 0LL) {
                fifo_ok = 0;
                rej_fifo_notime++;
            } else if (!FifoCoincidenceCheck(fd, ts_exp, last_used_ts1)) {
                fifo_ok = 0;
                rej_fifo++;
            } else {
                fifo_ok = 1;
    ...
    // Riepilogo finale
    int total_fifo_scartati = rej_fifo + rej_fifo_notime;
    int total_after_fifo_fin = total_events - total_fifo_scartati;
    std::cout << "\n=============================================" << std::endl;
    ...
    std::cout << "  Scartati (FIFO no coinc.): " << rej_fifo
              << " (" << pct(rej_fifo) << "%)" << std::endl;
    std::cout << "  Totale scartati dal FIFO:  " << total_fifo_scartati
              << " (" << pct(total_fifo_scartati) << "%)" << std::endl;
    std::cout << "  Dopo filtro FIFO:          " << total_after_fifo_fin << std::endl;
    std::cout << "  ───────── TUTTI GLI EVENTI ────────────" << std::endl;
    std::cout << "  Eventi buoni (slew):       " << total_good
              << " (" << pct(total_good) << "%)" << std::endl;
    std::cout << "  Eventi buoni (TOT):        " << total_good_tot
              << " (" << pct(total_good_tot) << "%)" << std::endl;
    std::cout << "  ───────── SOLO EVENTI LEAD ────────────" << std::endl;
    std::cout << "  Eventi buoni lead (slew):  " << total_good_lead
              << " (" << pct(total_good_lead) << "%)" << std::endl;
    std::cout << "  Eventi buoni lead (TOT):   " << total_good_tot_lead
              << " (" << pct(total_good_tot_lead) << "%)" << std::endl;
    std::cout << "  Output: " << outname << std::endl;
    std::cout << "=============================================" << std::endl;
}
